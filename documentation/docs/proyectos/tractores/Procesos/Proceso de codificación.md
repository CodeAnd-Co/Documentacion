...
|------|-------------|-------------|
| **Verificar el análisis** | Llenar la checklist de análisis, si hay un punto faltante llevar a cabo las acciones correctivas para asegurar que el análisis es correcto. | Codificador del requisito |
...
| | [EXP] Rehacer el diagrama de secuencia a papel para verificar que tan útil es su diseño. | Codificador del requisito |
| | Llenar la checklist de diseño y crear una copia en la carpeta de 'Checklists TracTech'. | Codificador del requisito | 
| **Seguir el proceso de RTM** | Abrir y seguir el proceso de RTM | Codificador del requisito |
| **Preparación para la codificación** | Manten abierto el diagrama de secuencia del requisito para basarte de él a la hora de programar y asegúrate | Codificador del requisito |
| **Inicio de codificación** | Asegurate de estar en la rama de develop, en el repositorio pertiente ejecutando: ```git branch```, si no estás en develop: ```git checkout develop```, después ejecuta: ```git pull origin``` y ```npm i```, finalmente ejecuta:  ```git checkout -b <NombreDeLaRama>```, recuerda seguir la estrategia de ramas | Codificador del requisito |
| **Selección de pareja** | En el único caso de que no exista persona con la que colaborar, es obligatorio hacer pair programming para el desarrollo de historias de usuario. | Codificador del requisito|
...
| | **3.** Asegúrate de que dentro de la carpeta 'data' exista la caprte 'repositorios' | Codificador del requisito |
...
| | Tener en la carpeta 'domain' una carpeta 'API' de tu modulo | Codificador del requisito | 
| | En esa carpeta debes crear tu moduloAPI.js en donde debes agregar las funciones que incorporen las llamadas al servidor. | Codificador del requisito |
...
| | Abre la carpeta scripts en framework > utils, y crear el archivo con el código de js que correra en el frontEnd y manejará el DOM o body. No debes incluir lógica más allá de lectura, escritura y asignación de funciones y valores de los elementos HTML | Codificador del requisito |
