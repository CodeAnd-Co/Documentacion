...
sequenceDiagram
    ...
    note over route, controller: Editar fórmula (PUT)
    Usuario->>route: Put/editar/:formulaId
    activate route
    route->>controller: editarFormula(id)
    deactivate route
    activate controller
    controller-)usecase: editarFormula(id)
    activate usecase
    usecase-)repository: editarFormula(id)
    activate repository
    repository-)model: editarFormula(id)
    activate model
    ...
    alt Caso exitoso
      rect Lightgreen
      controller--)view: response('Parametros modificados correctamente')
      activate view
      ...
      rect Pink
      controller--)view: response('Error al modificar parametros')
      deactivate controller
      activate view
